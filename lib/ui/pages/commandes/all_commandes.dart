...
class _CommandesPageState extends State<CommandesPage> {
    ...
    if (codeSearch) {
      for (Command i in commandesSource) {
        if (i.code.toString().contains(code)) {
          ...
          debugPrint("$logTrace ${i.type}");
        }
      }
      debugPrint("$logTrace ${commandes.length}");
      ...
      if ((int.tryParse(prixRange[0]) != null) &&
        ...
        debugPrint("$logTrace taille:${commandes.length}");
      }
      // else {
      ...
      for (Command i in commandesSource) {
        if (i.validated == valide) {
          commandes.add(i);
          debugPrint("$logTrace type:${i.type}");
        }
      }
    }
  ...
  Future<List<Command>> getCommandes(String restoId) async {
    ...
    List<Command> commandsNow = [];
    List<Command> commandsHier = [];
    List<Command> commandsSemaine = [];
    List<Command> commandsMois = [];
    var url = Uri.parse(Api.commandes +
        // (restoId != "" ? "?filter={restaurant:$restoId}" : "?filter:{}"));
        // (restoId != ""
        //     ? "?filter=" + json.encode({'restaurant': restoId})
        //     : "?filter:{}"));
        (restoId != "" ? '?filter={"restaurant":"$restoId"}' : '?filter={}'));
    ...
    try {
      ...
      setState(() {
        for (var i in jsonData) {
          debugPrint("$logTrace _id${i.runtimeType} ${i['_id'].toString()}");
          // if (i['items'] != null) {
      ...
      MyToast.showAlert("Erreur lors du chargment");
      setState(() {
        processing = false;
      });
      return [];
  ...
  List<Command> updateComsRange(
    ...
    if (start.isAtSameMomentAs(end)) {
      for (Command c in initial) {
        DateTime dateCreation = DateTime.parse(c.createdAt.split("T").first);
        if ((dateCreation.isAtSameMomentAs(start))) {
          coms.add(c);
        }
      }
    } else {
      for (Command c in initial) {
        DateTime dateCreation = DateTime.parse(c.createdAt.split("T").first);
        if ((dateCreation.isAfter(start)) && (dateCreation.isBefore(end))) {
          coms.add(c);
        }
      }
    }
                              ...
                              PopupMenuItem(
                                  ...
                                  onTap: () {
                                    setState(() {
                                      ...
                                      valideSearch = false;
                                      typeSearch = false;
                                      debugPrint("$logTrace $codeSearch");
                                    });
                                  }),
                              // PopupMenuItem(
