...
class EmportedCommandesPage extends StatefulWidget {
  const EmportedCommandesPage({Key? key}) : super(key: key);

  @override
  _EmportedCommandesPageState createState() => _EmportedCommandesPageState();
}

class _EmportedCommandesPageState extends State<EmportedCommandesPage> {
  ...
  void testDate() {
    ...
    DateTime demain = now.subtract(const Duration(days: 7));
    print(demain);

    var date2 = DateTime.parse("2022-02-02");
    print(date2);

    var date3 = date2.subtract(const Duration(days: 7));
    print(date3);
  ...
  List<Command> updateComsRange(
    ...
    if (start.isAtSameMomentAs(end)) {
      for (Command c in initial) {
        ...
        DateTime dateRetrait = DateTime.parse(c.dateRetrait.split("T").first);
        if ((dateRetrait.isAtSameMomentAs(start))) {
          coms.add(c);
        }
      }
    } else {
      for (Command c in initial) {
        // DateTime dateRetrait = DateTime.parse(c.createdAt.split("T").first);
        DateTime dateRetrait = DateTime.parse(c.dateRetrait.split("T").first);
        if ((dateRetrait.isAfter(start)) && (dateRetrait.isBefore(end))) {
          coms.add(c);
        }
      }
    }
